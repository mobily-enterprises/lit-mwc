...
const FormElementMixin = base => {
  return class Base extends base {
    ...
    constructor() {
      ...
      this.nativeValidationKeys = ['badInput', 'customError', 'patternMismatch', 'rangeOverflow', 'rangeUnderflow', 'stepMismatch', 'valueMissing', 'tooLong', 'tooShort', 'typeMismatch'];
    ...
    firstUpdated() {
      ...
      this.native.oninvalid = e => {
        ...
        const validity = e.target.validity; // Find which one flag in validity is raised
        ...
        if (translator) {
          this.shownValidationMessage = typeof translator === 'function' ? translator(e.target.validationMessage) : translator;
        } else {
...
var FormElementMixin$1 = {
  FormElementMixin: FormElementMixin
};
export { FormElementMixin$1 as $FormElementMixin, FormElementMixin };