...
const NativeReflectorMixin = base => {
  return class Base extends base {
    ...
    _setSubAttr(subAttr, attrValue) {
      const tokens = subAttr.split('::'); // Safeguard: if this.native is not yet set, it means that
    ...
    _reflectAttributesAndProperties() {
      const dst = this.native; // STEP #1: ATTRIBUTES FIRST
      ...
      const thisObserver = new MutationObserver(mutations => {
        mutations.forEach(mutation => {
          if (mutation.type === 'attributes') {
            const attr = mutation.attributeName; // Don't reflect forbidden attributes

            if (this.skipAttributes.indexOf(attr) !== -1) return; // Don't reflect attributes with ::
            ...
            newValue === null ? super.removeAttribute(attr) : super.setAttribute(attr, newValue);
          }
        });
      });
...
var NativeReflectorMixin$1 = {
  NativeReflectorMixin: NativeReflectorMixin
};
export { NativeReflectorMixin$1 as $NativeReflectorMixin, NativeReflectorMixin };