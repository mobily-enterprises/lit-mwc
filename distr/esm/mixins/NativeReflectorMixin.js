...
const NativeReflectorMixin=base=>{return class Base extends base{// eslint-disable-line
...
// explained shortly.
firstUpdated(){/* Find the native element */this.native=this.shadowRoot.querySelector("#native");/* Get the boot property values which may have been set before the element */ /* had a chance to listen to property changes */const bootProperties=this._getBootProperties(),bootPropertiesValues={};for(const prop of bootProperties){if("undefined"!==typeof this[prop]){bootPropertiesValues[prop]=this[prop]}}/* Reflect all attributes and properties */ /*  - all properties are reflected except some (listed in skipAttributes) */ /*  - only elected properties are reflected (listed in reflectProperties) */this._reflectAttributesAndProperties();/* Set the boot properties for the element */for(const prop of Object.keys(bootPropertiesValues)){this[prop]=bootPropertiesValues[prop]}}// As mentoned above, boot properties are defined in the element, but
// users are able to add more by setting the attribute
// `extra-boot-properties`:
_getBootProperties(){// Assign "boot properties". This is an unfortunate hack that is
// necessary in order to assign custom properties added *before* the
// observer was on
let bootProperties=this.bootProperties;/* Users can have attribute `extra-boot-properties` */ /* to add boot properties */const fromAttr=this.getAttribute("extra-boot-properties");if(fromAttr&&"string"===typeof fromAttr){bootProperties=[...bootProperties,...fromAttr.split(" ")]}return bootProperties}// From https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement
get reflectProperties(){return["accessKey","accessKeyLabel","contentEditable","isContentEditable","contextMenu ","dataset","dir","draggable","dropzone","hidden","inert","innerText","itemScope ","itemType","itemId ","itemRef","itemProp","itemValue ","lang","noModule","nonce","offsetHeight","offsetLeft","offsetParent","offsetTop","offsetWidth","properties","spellcheck","style","tabIndex","title","translate","attachInternals","blur","click","focus","forceSpellCheck"]}get skipAttributes(){return["id","style","class"]}get bootProperties(){return["value"]}getAttribute(attr){if(-1!==this.skipAttributes.indexOf(attr)){return super.getAttribute(attr)}const nativeAttribute=this.native.getAttribute(attr);if(null!==nativeAttribute)return nativeAttribute;// This shouldn't really happen, but it's here as a fallback
// TODO: Maybe delete it and always return the native's value regardless
return super.getAttribute(attr)}setAttribute(attr,value){// Set the attribute
super.setAttribute(attr,value);// Skip the ones in the skipAttributes list
if(-1!==this.skipAttributes.indexOf(attr))return;// Assign the same attribute to the contained native
// element, taking care of the 'nn' syntax
//
this._setSubAttr(attr,value)}removeAttribute(attr){// Set the attribute
super.removeAttribute(attr);// Skip the ones in the skipAttributes list
if(-1!==this.skipAttributes.indexOf(attr))return;// Assign the same attribute to the contained native
// element, taking care of the 'nn' syntax
//
this._setSubAttr(attr,null)}_setSubAttr(subAttr,attrValue){const tokens=subAttr.split("::");// Safeguard: if this.native is not yet set, it means that
...
if(1===tokens.length){null===attrValue?this.native.removeAttribute(subAttr):this.native.setAttribute(subAttr,attrValue);// Yes, :: is there: assign the attribute to the element with the
// corresponding ID
}else if(2===tokens.length){const dstElement=this.shadowRoot.querySelector(`#${tokens[0]}`);if(dstElement){null===attrValue?dstElement.removeAttribute(tokens[1]):dstElement.setAttribute(tokens[1],attrValue)}}}_reflectAttributesAndProperties(){const dst=this.native;// STEP #1: ATTRIBUTES FIRST
// Assign all starting attribute to the destination element
for(const attributeObject of this.attributes){const attr=attributeObject.name;if(-1!==this.skipAttributes.indexOf(attr))continue;this._setSubAttr(attr,super.getAttribute(attr))}// Observe changes in attribute from the source element, and reflect
// them to the destination element
const thisObserver=new MutationObserver(mutations=>{mutations.forEach(mutation=>{if("attributes"===mutation.type){const attr=mutation.attributeName;// Don't reflect forbidden attributes
if(-1!==this.skipAttributes.indexOf(attr))return;// Don't reflect attributes with ::
if(-1!==attr.indexOf("::"))return;// Check if the value has changed. If it hasn't, there is no
// point in re-assigning it, especially since the observer
// might have been triggered by this very mixin
const newValue=this.native.getAttribute(attr),thisValue=super.getAttribute(attr);if(newValue===thisValue)return;// Assign the new value
null===newValue?super.removeAttribute(attr):super.setAttribute(attr,newValue)}})});thisObserver.observe(this.native,{attributes:!0});// STEP #2: METHODS (as bound functions) AND PROPERTIES (as getters/setters)
const uniqProps=[...new Set(this.reflectProperties)];uniqProps.forEach(prop=>{Object.defineProperty(this,prop,{get:function(){if("function"===typeof dst[prop])return dst[prop].bind(dst);else return dst[prop]},set:function(newValue){if("function"===typeof dst[prop])return;const oldValue=dst[prop];// Set the new value
dst[prop]=newValue;// This is required by litElement since it won't
// create a setter if there is already one
this._requestUpdate(prop,oldValue)},configurable:!0,enumerable:!0})})}}};var NativeReflectorMixin$1={NativeReflectorMixin:NativeReflectorMixin};export{NativeReflectorMixin$1 as $NativeReflectorMixin,NativeReflectorMixin};