...
class EnForm extends ThemeableMixin('en-form')(NnForm) {
  ...
  async firstUpdated() {
    ...
    if (this.validateOnRender) {
      await this._allChildrenCompleted(); // Check validity
  ...
  _fetchEl(specificElement) {
    ...
    if (specificElement) {
      ...
      if (this.fetchingElement) {
        if (typeof this.fetchingElement === 'string') return this.querySelector(`#${this.fetchingElement}`);else return this.fetchingElement;
      } else {
  ...
  async submit(specificElement) {
    ...
    const method = elementMethod === null ? this.recordId ? 'PUT' : 'POST' : elementMethod; // Set the url, which will also depend on recordId
    ...
    if (networkError) {
      ...
      this.dispatchEvent(event); // Re-enable the elements
  ...
  async updated(changedProperties) {
    super.updated(); // If no-autoload is set to true, or there is no autoload or no recordId,
    // simply give up: nothing to do

    if (this.noAutoload || !changedProperties.has('recordId')) return; // Record ID must be "something"

    if (typeof this.recordId === 'undefined' || this.recordId === null) return; // Work out the action's URL, adding the record ID  at the end
    // (It will be a get)
