...
class EeAutocomplete extends ThemeableMixin('ee-autocomplete')(StyleableMixin(LitElement)) {
  ...
  _findTarget(target) {
    if (target !== null) {
      if (typeof target === 'string') return this.querySelector(`#${target}`);else if (typeof target === 'object') return target;
    } else {
  ...
  _findTargetForId(target) {
    if (target !== 'null') {
      if (typeof target === 'string') {
        return target === '' ? this.querySelector('[name]:not([no-submit])') : this.querySelector(`#${target}`);
  ...
  async _inputEvent(e) {
    ...
    if (!target) return; // There is more input: a new query will be made,
    ...
    if (networkError) {
      ...
      const event = new CustomEvent('autocomplete-error', {
        detail: {
          type: 'network'
        },
        bubbles: true,
      ...
      const errs = await response.json(); // Emit event to make it possible to tell the user via UI about the problem
      ...
      const v = await response.json(); // Emit event to make it possible to tell the user via UI about the problem

      const event = new CustomEvent('form-ok', {
        detail: {
          response
        },
        bubbles: true,
